...
<?php
	...
	$responses = array();	
	while ($row = mysqli_fetch_array($result)){
		$query2 = "select mt.name as topic_name, mc.name as category_name from mismatch_topic as mt INNER JOIN mismatch_category as mc USING (category_id) where mt.topic_id = '" . $row["topic_id"] . "'";
		...
		/*//$query2 = "select mismatch_topic.topic_id, mismatch_category.name from mismatch_topic INNER JOIN mismatch_category ON (mismatch_topic.category_id = mismatch_category.category_id) where mismatch_topic.topic_id = '" . $row["topic_id"] . "'" ;
		//$query2 = "select mismatch_topic.topic_id, mismatch_category.name from mismatch_topic INNER JOIN mismatch_category USING (category_id) where mismatch_topic.topic_id = '" . $row["topic_id"] . "'" ;
		$query2 = "select mt.topic_id, mc.name from mismatch_topic AS mt INNER JOIN mismatch_category as mc USING (category_id) where mt.topic_id = '" . $row["topic_id"] . "'" ;
		$result2 = mysqli_query($dbc, $query2) or die("query mysql error5");
		while ($row2 = mysqli_fetch_array($result2)){
			$query3 = "select name from mismatch_topic where topic_id = '" . $row2["topic_id"] . "'";
			$result3 = mysqli_query($dbc, $query3) or die("query mysql error6");
			if (mysqli_num_rows($result2) == 1){
				$row3 = mysqli_fetch_array($result3);
			
				$row["topic_name"] = $row3["name"];
				//cate gory name
