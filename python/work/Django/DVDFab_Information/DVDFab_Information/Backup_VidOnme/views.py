...
TB_NAME = "vidonme_information"

def display_vidonme_information(request):
    ...
    try:
        ...
        cur.close()
        conn.close()
    return render_to_response("display_vidonme_information.html", locals())


def main(request):
    return render_to_response("main.html", locals())

def display_all_vidonme_information(request, param):
    try:
        conn = sqlite3.connect(DB_PATH)
        conn.isolation_level = None
        cur = conn.cursor()    
        ...
        cur.close()
        conn.close()
    return render_to_response("display_all_vidonme_information.html", locals())


def display_all_vidonme_record(request):
    platform_name = request.GET.get("platform_name","").strip()
    if not platform_name:
        platform_name = "Win"
    try:
...
def display_all_vidonme(request):
    try:
        conn = sqlite3.connect(DB_PATH)
        conn.isolation_level = None
        cur = conn.cursor()    
        ...
        cur.close()
        conn.close()
    return render_to_response("display_all_vidonme.html", locals())

@csrf_exempt
def backup_vidonme(request):
    url_path = request.get_full_path()
    checkbox = request.POST.getlist('checkbox')
    platform_name = request.POST.get("platform_name", "")
    num_list = request.POST.getlist("num")
    if len(checkbox) < 1:
        return render_to_response("select_vidonme_one.html", locals())
    if len(checkbox) > 1:
        return render_to_response("select_vidonme_error.html", locals())
    try:
        conn = sqlite3.connect(DB_PATH)
        conn.isolation_level = None
        cur = conn.cursor()    
        ...
        if res[11].upper() == "WIN":
            backup_txt = "/Volumes/MacintoshHD/auto_package/Backup_Win_VidOn_Video_Toolkit.txt"
            fp = open(backup_txt, "w")
            for record in version_list:
                fp.write(record + "\n\r")
            fp.close()
        elif res[11].upper() == "MAC":
            backup_txt = "/Volumes/MacintoshHD/auto_package/Backup_Mac_VidOn_Video_Toolkit.txt"
            fp = open(backup_txt, "w")
            for record in version_list:
                fp.write(record + "\n\r")
            #fp.write(res[8])
    ...
    return render_to_response("backup_vidonme.html", locals())
