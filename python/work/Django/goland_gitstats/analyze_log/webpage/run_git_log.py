...
TEMP_FILE = "/home/goland/develop/goland_gitstats/product.txt"
...
#LOG_FILENAME = '/Users/DVDFab/goland_gitstats/analyze_log/webpage/run_git_log.log'
...
def get_all_projects(product_name):
    all_projects = []
    conn = MySQLdb.connect(host = DB_HOST, user = DB_USER, passwd = DB_PASSWORD, charset='utf8')   
    conn.select_db(DB_NAME)        
    cursor = conn.cursor()     
    select_sql = "select project_name from gitstats_product where name = '%s'" % product_name
    cursor.execute(select_sql)
    conn.commit()
    res = cursor.fetchone() 
    cursor.close()   
    conn.close()
    all_projects = [i.strip() for i in res[0].strip().split(";")]
    return all_projects
...
def get_last_commit_time_and_branch_name(each_project):
    ...
    if res:
        ...
        last_commit_time = "2015-11-30 00:00:00"
...
def git_clone_project(project_name):
    ...
    git_clone_cmd = 'echo "user"|sudo -S /usr/bin/git clone ' + project_name + " " + folder_name
...
def run_git_log(each_project, project_local_path, branch_name,last_commit_time, end_time,tempfile):
    ...
    p = subprocess.Popen(git_reset_cmd, cwd = project_local_path,stdout = subprocess.PIPE, stderr = subprocess.PIPE,  shell = True)
    out = p.stdout.read() 
    if out:
        print "out9999999: ", out 
        log("out99999999: %s" % out)
    err = p.stderr.read() 
    if err:
        log(git_reset_cmd+" 234567890")
        print "err8888888888: ", err
        log("err88888888888888: %s" % err)
    git_checkout_cmd = 'echo "user"|sudo -S /usr/bin/git checkout ' + branch_name
    subprocess.call(git_checkout_cmd, cwd = project_local_path, shell = True)
    git_pull_cmd = 'echo "user"|sudo -S /usr/bin/git pull'
    subprocess.call(git_pull_cmd, cwd = project_local_path, shell = True)
    log("branch_name is : %s" % branch_name)
    git_log_cmd = 'echo "user"|sudo -S /usr/bin/git log --branches="%s" --after="%s" --before="%s" > %s' % (branch_name,last_commit_time, end_time,TEMPFILE)
    ...
    err = p.stderr.read()
    if err:
        print "err111111111111111222222222: ", err
        log("project_name is : %s" % each_project)
        log("err11111111: %s" % err)
...
def main(all_projects):
    ...
    all_projects = get_all_projects(product_name)
    #all_projects = json.loads(all_projects)
    #all_projects = all_projects.split(";")
    print all_projects
    for each_project in all_projects:
        ...
        for branch_name in all_branches_list:
            log("\n---------------------------current branch is :%s-------------------------------" % branch_name)
            package_path = ""
            flag = 0
